...
import botbowl
from botbowl import Action, ActionType, Square, BBDieResult, Skill, Formation, ProcBot, OutcomeType, Player
...
class SearchBot(ProcBot):
    ...
    def turn(self, game):
        ...
        if root_node.action in self.records:
            ...
            for j in range(len(root_node.children)):
                ...
                while not game_copy.state.game_over and len(game_copy.state.available_actions) > 0:
                    # original_step = game_copy.get_step()
                    if times > self.threshold:
                        break
                    try:
                        while True:
                        ...
                        evaluate_score += self.evaluate_new(game_copy)
                        times += 1
        ...
        # game_copy.step(best_node.action)
        # print("action:", best_node.action)
        return best_node.action

    def evaluate_new(self, game):
