...
import botbowl
from botbowl import Action, ActionType, Square, BBDieResult, Skill, Formation, ProcBot, OutcomeType
...
class Node:
    ...
    def ubc(self):
        ...
        for sub_node in self.children:
            left = sub_node.score()
            right = math.sqrt(math.log(self.num_visits()) / sub_node.num_visits())
            score = left + C * right
            if score > best_score:
...
class SearchBot(ProcBot):
    ...
    def __init__(self, name):
        ...
        self.records = dict()
        self.rewards_own = {
            OutcomeType.TOUCHDOWN: 0,
            OutcomeType.SUCCESSFUL_CATCH: -1,
            OutcomeType.INTERCEPTION: -2,
            OutcomeType.SUCCESSFUL_PICKUP: -1,
            OutcomeType.FUMBLE: 1,
            OutcomeType.KNOCKED_DOWN: 1,
            OutcomeType.KNOCKED_OUT: 2,
            OutcomeType.CASUALTY: 5
        }
        self.ball_progression_reward = 0.005
    ...
    def turn(self, game):
        ...
        if root_node.action in self.records:
            ...
            for child in root_node.children:
                ...
                if child.action.action_type == best_action.action_type:
                    temp.append(child)
            if len(temp)>0:
                root_node.children = temp
            ...
            for j in range(len(root_node.children)):
                ...
                root_node.children[j].visit(score)
                print("score:", score)
                node_parent = root_node.children[j].parent
                while True:
                    node_parent.visit(score)
    ...
    def evaluate(self, game):
        ...
        # print("outcome:", len(set(game.state.reports)))
